...
const FAQ = ({ showHeading = true, className }: FAQProps) => {
  const faqData = [
    {
      question:
  ...
  return (
    <div
      className={cn(
        "min-h-screen bg-black px-6 py-16 text-white",
        className
      )}
    >
      {showHeading && (
...
const AccordionItem = ({ question, answer }: AccordionItemProps) => {
  ...
  return (
    <div className="overflow-hidden bg-black">
      <button
        className="flex w-full items-center justify-between bg-black p-5 text-left text-lg font-semibold transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <div
        className={cn(
          ...
          isOpen ? "max-h-screen opacity-100 p-5" : "max-h-0 px-5 opacity-0"
        )}
        style={{
