...
const { verifyOfferSignature, verifyCancelSignature } = require('../services/signature');
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const offers = await offersService.getOffers({ chain, token, min_amount, max_amount, limit, sort });
...
router.post('/', rateLimiter, async (req, res, next) => {
  try {
    ...
    const savedOffer = await offersService.createOrUpdateOffer(value, signature);
...
router.post('/:id/cancel', rateLimiter, async (req, res, next) => {
  try {
    ...
    if (!signature || nonce === undefined || nonce === null || !seller_pubkey) {
      return res.status(400).json({ error: 'Signature, nonce and seller_pubkey are required' });
    }

    const offerId = req.params.id;
    const nonceBigInt = BigInt(nonce);

    const isValidSignature = verifyCancelSignature(offerId, nonceBigInt, signature, seller_pubkey);
    if (!isValidSignature) {
      return res.status(401).json({ error: 'Invalid signature for cancellation' });
    }

    const cancelled = await offersService.cancelOffer(offerId, nonceBigInt, signature, seller_pubkey);
...
router.patch('/:id', rateLimiter, async (req, res, next) => {
  try {
    ...
    if (!signature || available_pyusd === undefined || available_pyusd === null || nonce === undefined || nonce === null || !seller_pubkey) {
      return res.status(400).json({ error: 'Signature, available_pyusd, nonce and seller_pubkey are required' });
    }

    const offerId = req.params.id;
    const nonceBigInt = BigInt(nonce);

    const updated = await offersService.updateAvailableAmount(
      offerId,
      {
        available_pyusd,
        nonce: nonceBigInt,
